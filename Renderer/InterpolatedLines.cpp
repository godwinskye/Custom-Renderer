...
void LineRenderer::LiDDArender3D(Drawable * drawable, OctantWiz::Point3D origin, OctantWiz::Point3D endpoint, Matrix & zBuffer, unsigned int color1, unsigned int color2, ViewFrustum view) {
	double gradient = MathWiz::GetGradient3D(origin, endpoint);
	OctantWiz::Point diffpoint(endpoint.x - origin.x, endpoint.y - origin.y);
	OctantWiz::Octant target = OctantWiz::FindOctant(diffpoint);
	...
	case OctantWiz::Octant::OctantOne:
	case OctantWiz::Octant::OctantEight:
		RenderOctant3D1Or8(drawable, origin, endpoint, zBuffer, gradient, color1, color2, view);
		break;
	case OctantWiz::Octant::OctantTwo:
	case OctantWiz::Octant::OctantThree:
		RenderOctant3D2Or3(drawable, origin, endpoint, zBuffer, gradient, color1, color2, view);
		break;
	case OctantWiz::Octant::OctantFour:
	case OctantWiz::Octant::OctantFive:
		RenderOctant3D4Or5(drawable, origin, endpoint, zBuffer, gradient, color1, color2, view);
		break;
	case OctantWiz::Octant::OctantSix:
	case OctantWiz::Octant::OctantSeven:
		RenderOctant3D6Or7(drawable, origin, endpoint, zBuffer, gradient, color1, color2, view);
		break;
	}
}
...
//View Frustum Renderers
void LineRenderer::RenderOctant3D1Or8(Drawable * drawable, OctantWiz::Point3D origin, OctantWiz::Point3D endpoint, Matrix & zBuffer, double gradient, unsigned int color1, unsigned int color2, ViewFrustum view) {
	double currentY = origin.y;
	...
	for (int x = origin.x; x <= endpoint.x; x++) {
		if (x < view.xlow || currentY > view.ylow || currentZ < view.hither || x > view.xhigh || currentY < view.yhigh || currentZ > view.yon) {
			currentY = currentY + gradient;
			currentZ = currentZ + dz;
...
void LineRenderer::RenderOctant3D2Or3(Drawable * drawable, OctantWiz::Point3D origin, OctantWiz::Point3D endpoint, Matrix & zBuffer, double gradient, unsigned int color1, unsigned int color2, ViewFrustum view) {
	double currentX = origin.x;
	double reversegradient = 1 / gradient;
	...
	for (int y = origin.y; y >= endpoint.y; y--) {
		if (currentX < view.xlow || y > view.ylow || currentZ < view.hither || currentX > view.xhigh || y < view.yhigh || currentZ > view.yon) {
			currentX = currentX - reversegradient;
			currentZ = currentZ + dz;
...
void LineRenderer::RenderOctant3D4Or5(Drawable * drawable, OctantWiz::Point3D origin, OctantWiz::Point3D endpoint, Matrix & zBuffer, double gradient, unsigned int color1, unsigned int color2, ViewFrustum view) {
	double currentY = origin.y;
...
void LineRenderer::RenderOctant3D6Or7(Drawable * drawable, OctantWiz::Point3D origin, OctantWiz::Point3D endpoint, Matrix & zBuffer, double gradient, unsigned int color1, unsigned int color2, ViewFrustum view) {
	double currentX = origin.x;
	double reversegradient = 1 / gradient;
