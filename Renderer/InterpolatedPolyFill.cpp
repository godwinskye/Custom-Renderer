...
void PolyFill::ScissorTriangle3D(Drawable * drawable, OctantWiz::Point3D origin, OctantWiz::Point3D endpoint1, OctantWiz::Point3D endpoint2, Matrix & zBuffer, unsigned int color1, unsigned int color2, unsigned int color3, ViewFrustum view) {
	std::vector<OctantWiz::Point3D> list;
	OctantWiz::Point3D top, middle, bottom;
	list.push_back(origin);
	list.push_back(endpoint1);
...
void PolyFill::DepthWireTriangle3D(Drawable * drawable, OctantWiz::Point3D origin, OctantWiz::Point3D endpoint1, OctantWiz::Point3D endpoint2, Matrix & zBuffer, unsigned int color1, unsigned int color2, unsigned int color3, ViewFrustum view) {
}

void PolyFill::DepthScissorTriangle3D(Drawable * drawable, OctantWiz::Point3D origin, OctantWiz::Point3D endpoint1, OctantWiz::Point3D endpoint2, Matrix & zBuffer, unsigned int color1, unsigned int color2, unsigned int color3, ViewFrustum view) {
	std::vector<OctantWiz::Point3D> list;
	OctantWiz::Point3D top, middle, bottom;
	list.push_back(origin);
	list.push_back(endpoint1);
	...
	for (int y = top.y + 1; y < middle.y; y++) {
		for (int x = leftpoint; x <= rightpoint - 1; x++) {
			if (x < view.xlow || y > view.ylow || 1 / currentZ < view.hither || x > view.xhigh || y < view.yhigh || 1 / currentZ > view.yon) {
				color.AddColor(colorgradient);
				currentZ += innerZgradient;
				continue;
			}
			if (1 / currentZ < zBuffer.at(x, y)) {
				drawable->setPixel(x, y, color.getHex());
				zBuffer.setAt(x, y, 1 / currentZ);
			}
			color.AddColor(colorgradient);
			currentZ += innerZgradient;
		}
